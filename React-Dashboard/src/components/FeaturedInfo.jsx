        ...
        const getincomes = async ()=>{
            try{
              ...
              console.log('res345', res.data);
                ...
                setSellPercent((res.data[1].quantity*100)/ res.data[0].quantity -100)                
              }
              catch{
              }
            }
    ...
    <div className="w-full flex justify-between">   
      <div className="flex-1 h-[150px] mx-5 p-7 shadow">
        ...
        <div className="my-4 items-center flex">
          ...
          {Math.floor(incomepercent)}% {(incomepercent>0)&&<ArrowUpward className="text-sm ml-1 text-green-500"/>}{(incomepercent<0)&&<ArrowDownward className="text-sm ml-1 text-red-500"/>}
          </span>
        </div>
      </div>
      <div className="flex-1 h-[150px] mx-5 p-7 shadow">
        <span className="flex text-2xl">فروش <h3 className="mr-2 text-teal-800 ">{monthDefini()}</h3></span>
        <div className="my-4 items-center flex">
          <span className="text-2xl"> {nf.format(income.quantity)} محصول </span>
          <span className="flex items-center mr-5 ltr">
          {Math.floor(sellpercent)}% {(sellpercent>0)&&<ArrowUpward className="text-sm ml-1 text-green-500"/>}{(sellpercent<0)&&<ArrowDownward className="text-sm ml-1 text-red-500"/>}
          </span>
        </div>
      </div>
    </div>
